 ...
 * 
 * Usage:   Stopwatch::start($name); Stopwatch::stop($name);
 *
 * @copyright Maxipes Fík
 */
final class Stopwatch implements Nette\IDebugPanel
{
    public static $timers = array();
    public static $timers_start = array();

    public static function start($name)
    {
        self::$timers_start[$name] = self::getMicrotime();
    }
    
    public static function stop($name)
    {
        $microtime = self::getMicrotime() - self::$timers_start[$name];
    self::add($microtime, $name);
    }

    public static function add($time, $name)
    {
        if ($name === NULL) $name = uniqid();
        if( isset(self::$timers[$name]) ) self::$timers[$name]+= $time;
        else self::$timers[$name] = $time;
    }
